...
router.delete("/:postid", async function (req, res, next) {
  ...
  postingModel.deleteOne({ postId: req.params.postid }, function (err) {
    ...
    commentModel.deleteMany({ postId: req.params.postid }, async function (err) {
      if (err) {
        res.status(500).send(false);
        return;
      }
      ...
      userModel.updateOne(
        ...
        function (err) {
          ...
          user.list.map(async (menteeId, i) => {
            let mentee = await userModel.findOne({ userID: menteeId });
            let filtered = []; 
            filtered = mentee.list.filter((element) => element !== req.session.user.userID);
            userModel.updateOne(
              { userID: menteeId },
              {
                list: filtered
...
router.post("/apply", async function (req, res, next) {
  ...
  let mentor = await userModel.find({
    nickname: post[0].userNickname,
    profileImage: post[0].userProfile,
    major: post[0].mentormajor,
  });
  ...
  let updateMentee = await userModel.updateOne(
  ...
  let updateMentor = await userModel.updateOne(
    { nickname: post[0].userNickname, profileImage: post[0].userProfile, major: post[0].mentormajor },
    { list: menteeList }
  );
